...
const Buttons = ({ tweet }) => {
  // oturumu açık olan kullanıcı bu tweet'i likelayanlar arsında mı
  const isLiked = tweet.likes.includes(auth.currentUser.uid);

  // like butonuna tıklanınca
  const toggleLike = () => {
    // güncellenecek tweet'in referansını alıyoruz
  ...
  return (
    <div className="flex justify-between items-center text-zinc-500">
      ...
      >
        ...
        <span className={`${isLiked && "text-pink-500"} absolute -end-2`}>
          {tweet.likes.length}
        </span>
      </button>
